...
If we want to infer cause and effect we can use linear regression (see [9.1.3.2 Cause and Effect](da_infer.qmd#sec-reg)). Let's say we want to know if the number of insects at a site is predictive of mammal mass at a site. First let's join the two data frames like wed did in the last chapter:
